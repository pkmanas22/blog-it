...
class SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_should_respond_with_not_found_error_if_user_is_not_present
    non_existent_email = "not_exist_in_db@example.email"
  ...
  def test_should_require_email_and_password_params
    post session_path,
      params: { login: { password: "password123" } },
      as: :json

    assert_response :bad_request
    response_json = response.parsed_body
    assert_includes response_json["error"], I18n.t("session.email_password_required")

    post session_path,
      params: { login: { email: @user.email } },
      as: :json
  ...
  def test_destroy_should_clear_current_user
    delete session_path,
